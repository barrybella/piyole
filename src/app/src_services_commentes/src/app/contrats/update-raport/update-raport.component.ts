...
@Component({
  selector: 'app-update-raport',
  templateUrl: './update-raport.component.html',
  styleUrls: ['./update-raport.component.css']
})
export class UpdateRaportComponent implements OnInit {
  user?: User;
  ...
  tables: any[] = [];
  tabs: any[] = [];
  constructor(public dialogRef: MatDialogRef<UpdateRaportComponent>,
  ...
  onSubmit(){
    ...
    if(+this.montant?.value){
      ...
      mont_calcule = (+this.mont_versement) - (+mont);
        ...
        if((+this.montant?.value) <= (+mont_calcule)){
          this.contratService.upload(formData).subscribe(res => {
            ...
            Swal.fire("IMPOSSIBLE ", "Le solde sera negatif!!", "error");
