...
@Component({
  selector: 'app-remove-payement',
  templateUrl: './remove-payement.component.html',
  styleUrls: ['./remove-payement.component.css']
})
export class RemovePayementComponent implements OnInit {
  post?: any;
  passwordIncorect?: boolean = false;
  etatPadding?: boolean = false;
  constructor(private fb: FormBuilder, private router: Router, public dialogRef: MatDialogRef<RemovePayementComponent>,
  ...
  onSubmit(){
    ...
    this.userService.confirmPasswordLogin(object).subscribe(res => {
      ...
      if(!res){
        ...
        this.contratService.removeReglement(this.data.contrat._id, this.data.month, this.data.year).subscribe(resp => {
